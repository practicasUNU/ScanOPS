...
/** KPI summary pill shown in the page header. */
...
/**
 * UnifiedScannerLayout — contenedor principal de la vista M2+M3.
 * Gestiona el estado de carga/error del hook y distribuye los datos
 * entre las tres pestañas.
 */
export function UnifiedScannerLayout() {
  const { data, loading, error, refetch } = useScanData();

  // Derive findings array from API shape or empty fallback
  const findings = data?.findings ?? data?.items ?? [];

  // Count by severity for header KPIs
  ...
  return (
    <div className="flex flex-col gap-6 p-6 min-h-screen bg-[#0f1117]">
      {/* ── Page header ─────────────────────────────────────────────────────── */}
